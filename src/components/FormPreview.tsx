...
const FormPreview: React.FC<FormPreviewProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    const saveSubmission = async () => {
      if (currentStep === totalSteps && !hasSubmitted.current) {
    ...
    <div className="fixed inset-0 z-50 text-white flex flex-col font-sans crt-overlay">
      ...
      <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start z-20 pointer-events-none">
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
          {!isPublic && (
            <div className="flex items-center gap-1 bg-black/40 backdrop-blur border border-white/10 rounded-lg p-1 mr-4">
              <button
                onClick={() => setPreviewMode('desktop')}
                className={`p-1.5 rounded-md transition ${previewMode === 'desktop' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-500 hover:text-white hover:bg-white/10'}`}
                title="Desktop View"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-md transition ${previewMode === 'tablet' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-500 hover:text-white hover:bg-white/10'}`}
                title="Tablet View"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-md transition ${previewMode === 'mobile' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-500 hover:text-white hover:bg-white/10'}`}
                title="Mobile View"
              >
          ...
          {!isPublic && <button onClick={onClose} className="px-4 py-1.5 border border-white/10 bg-black/40 backdrop-blur text-xs font-mono hover:bg-white/10 hover:border-white/30 transition uppercase rounded-lg">[ ESC ] Abort</button>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col items-center justify-center py-20 px-4 md:px-6 z-10 w-full">
        <div className={`w-full flex flex-col items-center justify-center transition-all duration-500 ease-out h-full overflow-hidden ${previewMode === 'mobile' ? 'max-w-[375px] h-[812px] border-[8px] border-slate-900 rounded-[3rem] shadow-2xl relative bg-[#050508]/80 backdrop-blur-md' :
          previewMode === 'tablet' ? 'max-w-[768px] h-[1024px] border-[8px] border-slate-900 rounded-[2rem] shadow-2xl relative bg-[#050508]/80 backdrop-blur-md' :
            'max-w-full'
          }`}>
          {previewMode !== 'desktop' && (
          ...
          <div className={`w-full h-full flex flex-col items-center justify-center p-4 md:p-8 overflow-y-auto custom-scrollbar ${previewMode !== 'desktop' ? 'pt-12' : ''}`}>
            {currentStep === -1 && (
